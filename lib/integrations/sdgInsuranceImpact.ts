...
// SDG Impact Dashboard Component
export function SDGInsuranceDashboard() {
  const { impact, metrics, stories, loading, error } = useSDGMetrics();
  ...
  if (error) {
    return (
      <div className="text-center text-red-600 p-8">
        Error loading SDG metrics: {error}
      </div>
    );
  }
  ...
  return (
    <div className="bg-gradient-to-br from-green-50 to-blue-50 p-8 rounded-2xl">
      <h2 className="text-3xl font-bold mb-8 text-center">Social Impact Dashboard</h2>
      
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
        <ImpactCard
          sdg={1}
          title="No Poverty"
          metric={`${impact.familiesProtected.toLocaleString()}K families`}
          description="Protected from financial catastrophe"
          icon="🛡️"
          value={metrics.sdg1.financialResilience}
        />
        
        <ImpactCard
          sdg={3}
          title="Good Health"
          metric={`${impact.healthAccess.toLocaleString()}K people`}
          description="Access to health coverage"
          icon="🏥"
          value={metrics.sdg3.healthOutcomes}
        />
        
        <ImpactCard
          sdg={8}
          title="Economic Growth"
          metric={`${impact.unbankedServed.toLocaleString()}K unbanked`}
          description="Financial inclusion achieved"
          icon="💼"
          value={metrics.sdg8.economicInclusion}
        />
        
        <ImpactCard
          sdg={13}
          title="Climate Action"
          metric={`${impact.farmersProtected.toLocaleString()}K farmers`}
          description="Climate risk protection"
          icon="🌾"
          value={metrics.sdg13.climateResilience}
        />
      </div>

      <div className="bg-white rounded-2xl p-6 mb-8">
        <h3 className="text-xl font-bold mb-4">Impact Stories</h3>
      ...
      <div className="bg-white rounded-2xl p-6">
        <h3 className="text-xl font-bold mb-4">SDG Alignment Summary</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <SDGProgressCard
            sdg={1}
            title="No Poverty"
            progress={metrics.sdg1.financialResilience}
            target={100}
          />
          <SDGProgressCard
            sdg={3}
            title="Good Health"
            progress={metrics.sdg3.healthOutcomes}
            target={100}
          />
          <SDGProgressCard
            sdg={8}
            title="Economic Growth"
            progress={metrics.sdg8.economicInclusion}
            target={100}
          />
          <SDGProgressCard
            sdg={13}
            title="Climate Action"
            progress={metrics.sdg13.climateResilience}
            target={100}
          />
        </div>
      </div>
...
function ImpactCard({ sdg, title, metric, description, icon, value }: any) {
  return (
    <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-200">
      <div className="flex items-center justify-between mb-4">
        <div className="text-3xl">{icon}</div>
        <div className="text-sm font-semibold text-gray-500">SDG {sdg}</div>
      </div>
      ...
      <div className="text-xs text-gray-500 mt-1">{value.toFixed(1)}% achieved</div>
    </div>
  );
}

// Impact Story Card Component
function ImpactStoryCard({ story }: any) {
  return (
    <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
      <div className="flex items-center gap-2 mb-2">
        <span className="text-sm font-semibold text-gray-500">SDG {story.sdg}</span>
        <span className="text-xs text-gray-400">•</span>
        <span className="text-xs text-gray-400">{story.region}</span>
      </div>
      <h4 className="font-semibold text-gray-900 mb-2">{story.title}</h4>
      <p className="text-sm text-gray-600 mb-3">{story.description}</p>
      <div className="text-xs text-green-600 font-semibold">{story.impact}</div>
    </div>
  );
}

// SDG Progress Card Component
function SDGProgressCard({ sdg, title, progress, target }: any) {
  ...
  return (
    <div className="bg-gray-50 rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-sm font-semibold text-gray-500">SDG {sdg}</span>
      </div>
